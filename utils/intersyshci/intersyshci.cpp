...
#ifdef __HCI_TRACE_BUFFER__NUMBER__
uint32_t send_completed_patcket= 0;
uint32_t send_data_number=0;
uint32_t received_data_number=0;
uint32_t received_complete_packet=0;
#endif
...
#else
#ifdef HCI_DEBUG
static uint16_t  buff_left=0;
uint8_t h4_event_buffer[1024];
#endif
static unsigned short INTERSYSHCI_LOC hci_h4_receive_msg( const uint8_t *buf, uint32_t size)
{
    ...
    while (bytes_read < size) {
        ...
        if (msg_received)
        {
             if(p_cb->rcv_msg_type == HCI_EVENT_PACKET){
#ifdef HCI_DEBUG
                h4_event_buffer[0] = 0x04;
                memcpy(&h4_event_buffer[1], p_buf->buffer, p_buf->len);
                DUMP8("%02x ", h4_event_buffer, p_buf->len + 1);
#endif
            }

#ifdef __HCI_TRACE_BUFFER__NUMBER__
            if(p_buf->flags == HCI_BUFTYPE_ACL_DATA)
            {
                received_data_number++;
                TRACE("rx %d",received_data_number);
            }
            else if(p_buf->flags == HCI_BUFTYPE_EVENT)
            {
                if(p_buf->buffer[0] == 0x13)
                {
                    received_complete_packet += p_buf->buffer[5];
                    TRACE("rxc %d,%d", p_buf->buffer[5],received_complete_packet);
                }
            }
#endif
...
void BESHCI_SendData(void *packet)
{
    ...
    if (pkt->flags == HCI_BUFTYPE_ACL_DATA) {
     //   DUMP8("%02x ",hci_senddata_buf,5);
        send_data_number++;
        TRACE("[Tn]:%d",send_data_number);

    }

    if (pkt->flags == HCI_BUFTYPE_COMMAND) {
        if(hci_senddata_buf[1] == 0x35 && hci_senddata_buf[2] == 0x0c)
        {
            send_completed_patcket += hci_senddata_buf[7];
            TRACE("[Tc:%d,%d",hci_senddata_buf[7],send_completed_patcket);
        }
    }
#endif
...
static unsigned int INTERSYSHCI_LOC bt_rx(const unsigned char *data, unsigned int len)
{
    ...
    intersys_sleep_checker_ping();
    status =  osMessagePut(intersys_rx_queue_id, 0, 0);
    if(status !=osOK)   
    {
        TRACE("error put rx msg");
    }
    OS_NotifyEvm();
    return len;
}
...
int BESHCI_SendBuffer(U8 packet_type, U8 *packet, int size)
{
    ...
    if (packet_type == 0x2) {
     //   DUMP8("%02x ",hci_senddata_buf,5);
        send_data_number++;
        TRACE("[Tnle]:%d",send_data_number);

    }

    if (packet_type == 0x01) {
        if(hci_ble_acl_buf[1] == 0x35 && hci_ble_acl_buf[2] == 0x0c)
        {
            send_completed_patcket += hci_ble_acl_buf[7];
            TRACE("[Tcle:%d,%d",hci_senddata_buf[7],send_completed_patcket);
        }
    }
#endif
    BESHCI_tx_flag = 4;
...
void INTERSYSHCI_LOC BESHCI_Poll(void)
{
    ...
    if (evt.status == osEventMessage) {
        if (!length){
#if 0            
            TRACE("[rX]:");
            DUMP8("%02x ",intersys_rx_buf.data, intersys_rx_buf.len);           
#endif            
            ...
            if(usedlen< intersys_rx_buf.len)
                TRACE("BESHCI_Poll leftlen=%x",intersys_rx_buf.len-usedlen);
        ...
        hal_intersys_rx_done(HAL_INTERSYS_ID_0);
         ...
         TRACE("BESHCI_Poll qlen=%x",length);
        nRet = PeekCQueue(&intersys_rx_queue, length, &e1, &len1, &e2, &len2);
        ASSERT(nRet == CQ_OK && !len2, "rx_thread PeekCQueue error");
        usedlen = hci_h4_receive_msg(e1, len1);
        DeCQueue(&intersys_rx_queue, 0, usedlen);
