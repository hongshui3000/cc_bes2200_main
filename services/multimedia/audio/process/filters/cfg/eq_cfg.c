...
#ifdef EQ_HW_DAC_IIR_LIST_NUM
#if (EQ_HW_DAC_IIR_LIST_NUM < 1)
#error "Invalid EQ_HW_DAC_IIR_LIST_NUM configuration"
#endif
#else
#define EQ_HW_DAC_IIR_LIST_NUM                   (1)
#endif

#ifdef EQ_HW_ADC_IIR_LIST_NUM
#if (EQ_HW_ADC_IIR_LIST_NUM < 1)
#error "Invalid EQ_HW_ADC_IIR_LIST_NUM configuration"
#endif
#else
...
extern const IIR_CFG_T * const audio_eq_hw_dac_iir_cfg_list[EQ_HW_DAC_IIR_LIST_NUM];
extern const IIR_CFG_T * const audio_eq_hw_adc_iir_cfg_list[EQ_HW_ADC_IIR_LIST_NUM];
...
HW_CODEC_IIR_CFG_T *hw_codec_iir_get_cfg(enum AUD_SAMPRATE_T sample_rate, HW_CODEC_IIR_TYPE_T hw_iir_type, int eq_index, IIR_CFG_T* ext_cfg)
{
    const IIR_CFG_T *cfg;
    ...
    if(eq_index<0||eq_index>=EQ_HW_DAC_IIR_LIST_NUM)return NULL;

    if (ext_cfg)
    {
        cfg = ext_cfg;
    }
    else if(HW_CODEC_IIR_DAC==hw_iir_type)
    {
        cfg = audio_eq_hw_dac_iir_cfg_list[eq_index];
    }
    else if(HW_CODEC_IIR_ADC==hw_iir_type)
    {
        cfg = audio_eq_hw_adc_iir_cfg_list[eq_index];
    }
    else
    {
        TRACE("Error IIR CODEC TYPE!");
        return NULL;
    }

    // Check parameter
    ASSERT(cfg->num <= AUD_IIR_NUM, "[%s] num(%d) is too large", __func__, cfg->num);
    ...
    for(int i=0; i<cfg->num; i++)
    {
        ...
        hw_codec_iir_eq.iir_filtes_r.iir_coef[i].coef_a[0]=(int32_t)((double)coefs_l[0]*(int32_t)IIR_FIXED_Q);
        hw_codec_iir_eq.iir_filtes_r.iir_coef[i].coef_a[1]=(int32_t)((double)coefs_l[1]*(int32_t)IIR_FIXED_Q);
        hw_codec_iir_eq.iir_filtes_r.iir_coef[i].coef_a[2]=(int32_t)((double)coefs_l[2]*(int32_t)IIR_FIXED_Q);
        hw_codec_iir_eq.iir_filtes_r.iir_coef[i].coef_b[0]=(int32_t)((double)coefs_l[3]*(int32_t)IIR_FIXED_Q);
        hw_codec_iir_eq.iir_filtes_r.iir_coef[i].coef_b[1]=(int32_t)((double)coefs_l[4]*(int32_t)IIR_FIXED_Q);
        hw_codec_iir_eq.iir_filtes_r.iir_coef[i].coef_b[2]=(int32_t)((double)coefs_l[5]*(int32_t)IIR_FIXED_Q);
