...
int anccfg_loadfrom_audsec(const struct_anc_cfg *list[], const struct_anc_cfg *list_44p1k[], uint32_t count)
{
    ...
    TRACE("Valid aud section.");
    for(i=0;i<APPLICATION_MODE_COUNT;i++)
        list[i] = (struct_anc_cfg *)&(audsec_ptr->sec_body.anc_config.anc_config_arr[active_app_mode_index].anc_cfg[SAMPLERATE_48X8K]);
    for(i=0;i<APPLICATION_MODE_COUNT;i++)
        list_44p1k[i] = (struct_anc_cfg *)&(audsec_ptr->sec_body.anc_config.anc_config_arr[active_app_mode_index].anc_cfg[SAMPLERATE_44_1X8K]);
