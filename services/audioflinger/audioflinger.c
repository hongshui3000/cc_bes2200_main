...
#elif defined(AUDIO_OUTPUT_DC_CALIB_ANA) && defined(__TWS__)
#define AUDIO_OUTPUT_PA_ON_FADE_IN
...
static enum AUD_SAMPRATE_T af_anc_samp_rate[AUD_STREAM_NUM];
static AF_ANC_HANDLER af_anc_handler;
...
enum AUD_SAMPRATE_T sample_rate_to_enum(uint32_t rate)
{
    if (rate == 192000) {
        ...
        ASSERT(false, "%s: Invalid sample rate: %u", __FUNCTION__, rate);
    }
...
#ifdef AUDIO_OUTPUT_DC_CALIB_SW
        int32_t dc_l = dac_dc[0] << (CODEC_PLAYBACK_BIT_DEPTH - 16);
        int32_t dc_r = dac_dc[1] << (CODEC_PLAYBACK_BIT_DEPTH - 16);
#endif
        if (chans == AUD_CHANNEL_NUM_1) {
            cnt = size / sizeof(int32_t);
            while (cnt-- > 0) {
#ifdef CORRECT_SAMPLE_VALUE
                *ptr32 = ((*ptr32) << (32 - CODEC_PLAYBACK_BIT_DEPTH)) >> (32 - CODEC_PLAYBACK_BIT_DEPTH);
#endif
#ifdef AUDIO_OUTPUT_SW_GAIN
                // Update the gain
                if (new_output_coef[0] != cur_output_coef[0]) {
...
#endif
#ifdef AUDIO_OUTPUT_DC_CALIB_SW
                *ptr32 = __SSAT(*ptr32 + dc_l, CODEC_PLAYBACK_BIT_DEPTH);
#elif defined(CORRECT_SAMPLE_VALUE)
                *ptr32 = __SSAT(*ptr32, CODEC_PLAYBACK_BIT_DEPTH);
#endif
                ptr32++;
            }
        } else {
...
#ifdef CORRECT_SAMPLE_VALUE
                *ptr32 = ((*ptr32) << (32 - CODEC_PLAYBACK_BIT_DEPTH)) >> (32 - CODEC_PLAYBACK_BIT_DEPTH);
                *(ptr32 + 1) = ((*(ptr32 + 1)) << (32 - CODEC_PLAYBACK_BIT_DEPTH)) >> (32 - CODEC_PLAYBACK_BIT_DEPTH);
#endif
#ifdef AUDIO_OUTPUT_SW_GAIN
                // Update the gain
                if (new_output_coef[0] != cur_output_coef[0]) {
...
#ifdef AUDIO_OUTPUT_DC_CALIB_SW
                *ptr32 = __SSAT(*ptr32 + dc_l, CODEC_PLAYBACK_BIT_DEPTH);
                *(ptr32 + 1) = __SSAT(*(ptr32 + 1) + dc_r, CODEC_PLAYBACK_BIT_DEPTH);
#elif defined(CORRECT_SAMPLE_VALUE)
                *ptr32 = __SSAT(*ptr32, CODEC_PLAYBACK_BIT_DEPTH);
                *(ptr32 + 1) = __SSAT(*(ptr32 + 1), CODEC_PLAYBACK_BIT_DEPTH);
#endif
                ptr32 += 2;
            }
        }
...
static inline void af_thread_stream_handler(enum AUD_STREAM_ID_T id, enum AUD_STREAM_T stream)
{
    ...
    if (role->handler && (role->ctl.status & AF_STATUS_STREAM_START_STOP)) {
        ...
        if (hw_pos < role->dma_buf_size / 2) {
            ...
            buf = role->dma_buf_ptr + PP_PINGPANG(role->ctl.pp_index) * (role->dma_buf_size / 2);
...
static void af_stream_update_dma_buffer(enum AUD_STREAM_T stream, struct af_stream_cfg_t *role, const struct AF_STREAM_CONFIG_T *cfg)
{
    ...
    if (role->ctl.use_device == AUD_STREAM_USE_INT_CODEC && cfg->chan_sep_buf &&
            role->cfg.channel_num > AUD_CHANNEL_NUM_1) {
        enum HAL_DMA_BSIZE_T bsize;
        uint8_t burst_size, samp_size;
        ...
        align = burst_size * samp_size * role->cfg.channel_num;
    ...
    ASSERT((desc_xfer_size % align) == 0, "%s: Desc xfer size is not aligned: buf_size=%u cnt=%u align=%u",
        __func__, role->dma_buf_size, AUDIO_BUFFER_COUNT, align);

    if (cfg->bits == AUD_BITS_24 || cfg->bits == AUD_BITS_32) {
        dma_cfg->dst_width = HAL_DMA_WIDTH_WORD;
        dma_cfg->src_width = HAL_DMA_WIDTH_WORD;
        dma_cfg->src_tsize = desc_xfer_size / 4; // word width
    } else if (cfg->bits == AUD_BITS_16) {
        dma_cfg->dst_width = HAL_DMA_WIDTH_HALFWORD;
        dma_cfg->src_width = HAL_DMA_WIDTH_HALFWORD;
        dma_cfg->src_tsize = desc_xfer_size / 2; // half word width
    } else {
...
uint32_t af_stream_setup(enum AUD_STREAM_ID_T id, enum AUD_STREAM_T stream, const struct AF_STREAM_CONFIG_T *cfg)
{
    ...
    else if(device == AUD_STREAM_USE_INT_CODEC)
    {
        ...
        // Invoke ANC callback
        if (af_anc_handler) {
            if (cfg->sample_rate != af_anc_samp_rate[stream]) {
                TRACE("[%s] ANC change sample rate from %u to %u", __func__, af_anc_samp_rate[stream], cfg->sample_rate);
                af_anc_handler(stream, cfg->sample_rate);
            }
        }
...
#ifndef CHIP_BEST1000
    if (device == AUD_STREAM_USE_INT_CODEC && role->cfg.chan_sep_buf) {
        ...
        if (src || dst) {
            ...
            dma_2d_cfg.xmodify = role->dma_cfg.src_tsize * AUDIO_BUFFER_COUNT / dma_2d_cfg.xcount - burst_size;
            dma_2d_cfg.ycount = role->dma_cfg.src_tsize / dma_2d_cfg.xcount / burst_size;
            dma_2d_cfg.ymodify = -(role->dma_cfg.src_tsize * AUDIO_BUFFER_COUNT / dma_2d_cfg.xcount) * (dma_2d_cfg.xcount - 1);
        }
...
uint32_t af_stream_get_cur_dma_addr(enum AUD_STREAM_ID_T id, enum AUD_STREAM_T stream)
{
    ...
    if(role->ctl.status == (AF_STATUS_OPEN_CLOSE | AF_STATUS_STREAM_OPEN_CLOSE | AF_STATUS_STREAM_START_STOP))
    {
        if (role->dma_cfg.ch != HAL_DMA_CHAN_NONE) {
            if (stream == AUD_STREAM_PLAYBACK) {
                return hal_audma_get_cur_src_addr(role->dma_cfg.ch);
            } else {
                return hal_audma_get_cur_dst_addr(role->dma_cfg.ch);
            }
        }
    }
...
int af_anc_open(enum ANC_TYPE_T type, enum AUD_SAMPRATE_T play_rate, enum AUD_SAMPRATE_T capture_rate, AF_ANC_HANDLER handler)
{
    ...
    af_anc_samp_rate[AUD_STREAM_PLAYBACK] = play_rate;
    af_anc_samp_rate[AUD_STREAM_CAPTURE] = capture_rate;
    af_anc_handler = handler;

    if (handler) {
        for (id = AUD_STREAM_ID_0; id < AUD_STREAM_ID_NUM; id++) {
            for (stream = AUD_STREAM_PLAYBACK; stream < AUD_STREAM_NUM; stream++) {
                role = af_get_stream_role(id, stream);
                if ((role->ctl.status & AF_STATUS_STREAM_OPEN_CLOSE) &&
                        role->ctl.use_device == AUD_STREAM_USE_INT_CODEC) {
                    cfg = &role->cfg;
                    if (cfg->sample_rate != af_anc_samp_rate[stream]) {
                        handler(stream, cfg->sample_rate);
                    }
                }
            }
        }
    ...
    codec_anc_open(type, af_anc_samp_rate[AUD_STREAM_PLAYBACK], af_anc_samp_rate[AUD_STREAM_CAPTURE]);
...
int af_anc_setup(enum AUD_SAMPRATE_T play_rate, enum AUD_SAMPRATE_T capture_rate)
{
    AF_TRACE_DEBUG();
