...
{0x191,0x15de,1},
//{0x194,0x0000,1},//rx current ℅⊿那赤米?㏒??谷豕??¯那㊣?車
{0x19a,0x6ecc,1},
...
{0xba,0x4000,1},
{0xbc,0xd120,1},//{0xbc,0x4920,1},//log buffer
...
{0x18f,0xdc00,1},//enhance dac driver
{0x191,0x15de,1},
//{0x194,0x8000,1},//rx current ℅⊿那赤米?㏒??谷豕??¯那㊣?車
{0x19a,0x6ecc,1},
{0x1e3,0x032f,1},//rf timing
{0x1ec,0x0190,1},//pu log & adc clk
{0x1f9,0x3c00,1},//pu tx en
...
{0xba,0x4000,1},
{0xbc,0xd120,1},//{0xbc,0x4920,1},//log buffer
{0xbd,0x0623,1},//{0xbd,0x0621,1},//set div ratio for adc
...
#ifdef __ADC_CAP_CALIB__
void btdtv_rf_2200_adc_cap_calib(void)
{
    if(hal_get_chip_metal_id() >= HAL_CHIP_METAL_ID_2)
    {
        uint16_t read_val;
        //read calibrated val from efuse 0x08 register         
        pmu_get_efuse(PMU_EFUSE_PAGE_RESERVED_8, &read_val);
        if(read_val == 0)
        {
            TRACE("EFUSE REG[0x08] has not been calibrated!");
            return;
        }
        else
        {
            TRACE("EFUSE REG[0x08]=%x",read_val);
        }
        read_val &= 0x1f;
        int16_t calib_val = ADC_CAP_REF_VAL - read_val;
        if(calib_val<0)
        {
            TRACE("ADC calib ERROR =%x,read=%x",ADC_CAP_REF_VAL,read_val);
            return;
        }
        else
        {
            TRACE("read calib val =%x",calib_val);
        }
        
        //write calibrated value into 0xDF register
        btdrv_read_rf_reg(0xdf,&read_val);
        read_val&=0xfe0f;
        read_val|=((calib_val&0x1e)<<4);
        TRACE("write adc calib val=%x in REG[0xDF]",read_val);
        btdrv_write_rf_reg(0xdf,read_val);
    }
}
#endif//__ADC_CAP_CALIB__
