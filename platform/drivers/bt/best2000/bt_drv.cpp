...
extern int32_t btdrv_syn_get_offset_ticks(uint16_t conidx)
{
    int32_t offset;
...
extern "C" uint8_t btdrv_conhdl_to_linkid(uint16_t connect_hdl)
{
    ...
    if(hal_get_chip_metal_id()>=HAL_CHIP_METAL_ID_2)
    {
     ...
     //   TRACE("hdl=%x,id=%x",connect_hdl,res);
        return res;
    }
    else
    {
     //   TRACE("hdl=%x,id=%x",connect_hdl,(connect_hdl-0x80));
...
{
    return BTDIGITAL_REG(0xd0220244);
}

uint32_t btdrv_get_locked_native_clk(void)
{
    return BTDIGITAL_REG(0xd0220248);
}

void btdrv_enable_dma_lock_clk(void)
{
    BTDIGITAL_REG(0x400000f0) |=  (0<<0)|(1<<3);//enable dma lock clock
}

void btdrv_detach_reset_status(void)
{
    TRACE("0xc00065c0=%x",*(uint32_t *)0xc00065c0);
...
{
    *(unsigned int *)0xc0004c1c = 0;
    *(unsigned int *)0xc0004c20 = 0;
    *(unsigned int *)0xc0004c10 = 0;
    *(unsigned int *)0xc0004c14 = 0;
