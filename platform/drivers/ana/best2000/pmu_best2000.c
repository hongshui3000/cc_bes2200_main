...
#define PMU_USB_POL_RX_DM_SHIFT         (15)
#define PMU_USB_POL_RX_DM               (1 << PMU_USB_POL_RX_DM_SHIFT)
#define PMU_USB_POL_RX_DP_SHIFT         (14)
#define PMU_USB_POL_RX_DP               (1 << PMU_USB_POL_RX_DP_SHIFT)
...
#define PMU_USB_STATUS_RX_DM_SHIFT      (10)
#define PMU_USB_STATUS_RX_DM            (1 << PMU_USB_STATUS_RX_DM_SHIFT)
#define PMU_USB_STATUS_RX_DP_SHIFT      (9)
#define PMU_USB_STATUS_RX_DP            (1 << PMU_USB_STATUS_RX_DP_SHIFT)
...
static uint8_t dig_act_ldo;
static const uint8_t dig_lp_ldo = PMU_VDIG_0_8V;
static const uint8_t dig_lp_dcdc = PMU_DCDC_DIG_0_9V;
...
#endif
    pmu_read(0xC5, &val);
    val = (val & ~((0xF << 0) | (0xF << 4))) | (4 << 0) | (4 << 4) | (1 << 8) | (1 << 9);   // BES asked to change this line by yangbin 20180126
    pmu_write(0xC5, val);
...
static void pmu_ldo_mode_en(void)
{
    ...
    hal_sys_timer_delay_us(1800); //delay1800us, 解决关机时候切换ldo延时不够的异常
...
static void pmu_dcdc_dual_mode_en(void)
{
    ...
    if (ret == 0) {
        ...
        do{
            pmu_write(PMU_REG_DCDC_CFG, val);
            pmu_read(PMU_REG_DCDC_CFG, &val_r);
            if((val_r & (PMU_DCDC_DIG_DR | PMU_DCDC_DIG_EN)) == 0){
                failed_cnt++;
                TRACE("******pmu_dcdc_dual_mode_en failed cnt:%d*****\n", failed_cnt);
            }else{
                break;
            }
        }while(1);
...
int BOOT_TEXT_FLASH_LOC pmu_open(void)
{
    ...
    ASSERT(vcodec_mv >= 2500 ||
            ...
            (vcodec_mv == 1600 && ana_act_dcdc == PMU_DCDC_ANA_1_7V) ||
    ...
    // enable reg_cp_en_avdd_sel by jeff
    ret = pmu_read(0x1C, &val);
    if (ret == 0) {
        val |= 1<<3;
        pmu_write(0x1C, val);
    }
...
void pmu_usb_enable_pin_status_check(void)
{
    if (usb_pin_status != PMU_USB_PIN_CHK_NONE && usb_pin_callback) {
        pmu_write(PMU_REG_INT_CLR, PMU_INT_CLR_USB_INSERT);
        NVIC_ClearPendingIRQ(PMU_USB_IRQn);
...
void pmu_usb_get_pin_status(int *dp, int *dm)
{
    ...
    *dm = ((pol & PMU_USB_POL_RX_DM) >> PMU_USB_POL_RX_DM_SHIFT) ^
