...
static int usbphy_reg_read(unsigned short reg, unsigned short *val)
{
    int ret;
    unsigned int data;
    unsigned int cmd;
...
static int usbphy_reg_write(unsigned short reg, unsigned short val)
{
    int ret;
    unsigned int cmd;
...
void usbphy_open(void)
{
    ...
    usbphy_reg_read(0x01, &val_01);
    // cdr_clk polariy=0 (stable for vcore 1.3V)
    val_01 &= ~((1 << 0) | (1 << 4) | (1 << 11));
    usbphy_reg_write(0x01, val_01);
    val_01 |= (1 << 1) | (1 << 2) | (1 << 13);
    usbphy_reg_write(0x01, val_01);

    usbphy_reg_read(0x02, &val);
    val = (val & ~(0x1F << 10)) | (0xC << 10);
    usbphy_reg_write(0x02, val);

    usbphy_reg_read(0x05, &val);
    val |= (1 << 8);
    usbphy_reg_write(0x05, val);

    // Tx off during rx
    usbphy_reg_read(0x06, &val);
    val |= (1 << 13) | (1 << 14);
    // Set hs_ibias_trim to 7 (default is 4)
    //val = (val & ~(7 << 4)) | (7 << 4);
    usbphy_reg_write(0x06, val);

    usbphy_reg_read(0x07, &val);
    val &= ~(1 << 8);
    usbphy_reg_write(0x07, val);

    usbphy_reg_read(0x08, &val);
    val = (val & ~((0xF << 0) | (0x3 << 4) | (0x3 << 6))) | (0x0 << 0) | (0x3 << 4) | (0x1 << 6);
    usbphy_reg_write(0x08, val);

    usbphy_reg_read(0x09, &val);
    val &= ~(1 << 0);
    val |= (1 << 3);
    usbphy_reg_write(0x09, val);

    // Disable force clocks, and disable hs son signal
    usbphy_reg_read(0x22, &val);
    val &= ~((1 << 4) | (1 << 5) | (1 << 6) | (1 << 7));
    usbphy_reg_write(0x22, val);

    // Ignore all UTMI errors
    usbphy_reg_write(0x12, 0x0003);
    ...
    usbphy_reg_read(0x0A, &val);
    val |= (1 << 3) | (1 << 4) | (1 << 6);
    usbphy_reg_write(0x0A, val);
    usbphy_reg_read(0x0B, &val);
    val &= ~((1 << 3) | (1 << 4) | (3 << 6));
    usbphy_reg_write(0x0B, val);
    usbphy_reg_read(0x09, &val);
    val |= (1 << 2);
    usbphy_reg_write(0x09, val);
    usbphy_reg_read(0x06, &val);
...
#define LOOPBACK_PATTERN                (*(volatile uint8_t *)0x20000004)
    ...
    // Set tx pattern
    usbphy_reg_read(0x03, &val);
    val = (val & ~(0xFF << 8)) | (pattern << 8);
    usbphy_reg_write(0x03, val);
    ...
    // Enable tx
    usbphy_reg_read(0x04, &val);
    val |= (1 << 1);
    usbphy_reg_write(0x04, val);
...
void usbphy_sleep(void)
{
    ...
    usbphy_reg_read(0x01, &val);
    val &= ~((1 << 1) | (1 << 2) | (1 << 3));
    usbphy_reg_write(0x01, val);
#endif
}
