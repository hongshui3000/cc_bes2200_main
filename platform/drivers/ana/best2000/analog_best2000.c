...
#if defined(ANC_TEST) && defined(ANC_FF_ENABLED) && ((ANC_FF_MIC_CH_L == AUD_CHANNEL_MAP_CH0) || (ANC_FF_MIC_CH_R == AUD_CHANNEL_MAP_CH0))
#define ANALOG_ADC_A_GAIN_DB                DEFAULT_ANC_FF_ADC_GAIN_DB
#elif defined(ANC_TEST) && defined(ANC_FB_ENABLED) && ((ANC_FB_MIC_CH_L == AUD_CHANNEL_MAP_CH0) || (ANC_FB_MIC_CH_R == AUD_CHANNEL_MAP_CH0))
...
#if defined(ANC_TEST) && defined(ANC_FF_ENABLED) && ((ANC_FF_MIC_CH_L == AUD_CHANNEL_MAP_CH1) || (ANC_FF_MIC_CH_R == AUD_CHANNEL_MAP_CH1))
#define ANALOG_ADC_B_GAIN_DB                DEFAULT_ANC_FF_ADC_GAIN_DB
#elif defined(ANC_TEST) && defined(ANC_FB_ENABLED) && ((ANC_FB_MIC_CH_L == AUD_CHANNEL_MAP_CH1) || (ANC_FB_MIC_CH_R == AUD_CHANNEL_MAP_CH1))
...
#if defined(ANC_TEST) && defined(ANC_FF_ENABLED) && ((ANC_FF_MIC_CH_L == AUD_CHANNEL_MAP_CH2) || (ANC_FF_MIC_CH_R == AUD_CHANNEL_MAP_CH2))
#define ANALOG_ADC_C_GAIN_DB                DEFAULT_ANC_FF_ADC_GAIN_DB
#elif defined(ANC_TEST) && defined(ANC_FB_ENABLED) && ((ANC_FB_MIC_CH_L == AUD_CHANNEL_MAP_CH2) || (ANC_FB_MIC_CH_R == AUD_CHANNEL_MAP_CH2))
...
#if defined(ANC_TEST) && defined(ANC_FF_ENABLED) && ((ANC_FF_MIC_CH_L == AUD_CHANNEL_MAP_CH3) || (ANC_FF_MIC_CH_R == AUD_CHANNEL_MAP_CH3))
#define ANALOG_ADC_D_GAIN_DB                DEFAULT_ANC_FF_ADC_GAIN_DB
#elif defined(ANC_TEST) && defined(ANC_FB_ENABLED) && ((ANC_FB_MIC_CH_L == AUD_CHANNEL_MAP_CH3) || (ANC_FB_MIC_CH_R == AUD_CHANNEL_MAP_CH3))
...
#if defined(ANC_TEST) && defined(ANC_FF_ENABLED) && ((ANC_FF_MIC_CH_L == AUD_CHANNEL_MAP_CH4) || (ANC_FF_MIC_CH_R == AUD_CHANNEL_MAP_CH4))
#define ANALOG_ADC_E_GAIN_DB                DEFAULT_ANC_FF_ADC_GAIN_DB
#elif defined(ANC_TEST) && defined(ANC_FB_ENABLED) && ((ANC_FB_MIC_CH_L == AUD_CHANNEL_MAP_CH4) || (ANC_FB_MIC_CH_R == AUD_CHANNEL_MAP_CH4))
...
static uint16_t main_adc_gain;
static int16_t main_adc_gain_offset;
#endif
...
#ifdef ANC_TEST
static int8_t dac_gain_offset;
static int8_t adc_gain_offset[2][2];
#endif
    ...
    for (i = 0; i < MAX_ANA_MIC_CH_NUM; i++) {
        if (ch_map & (AUD_CHANNEL_MAP_CH0 << i)) {
            if (input_path == AUD_INPUT_PATH_LINEIN) {
                gain = qdb_to_adc_gain(LINEIN_ADC_GAIN_DB * 4);
            } else {
                gain = tgt_adc_qdb[i];
#ifdef ANC_TEST
#ifdef ANC_FF_ENABLED
                if ((ANC_FF_MIC_CH_L | ANC_FF_MIC_CH_R) & (AUD_CHANNEL_MAP_CH0 << i)) {
                    int lr = (ANC_FF_MIC_CH_L & (AUD_CHANNEL_MAP_CH0 << i)) ? 0 : 1;
                    if (adc_gain_offset[0][lr] < 0 && -adc_gain_offset[0][lr] > gain) {
                        gain = 0;
                    } else {
                        gain += adc_gain_offset[0][lr];
                    }
                }
#endif
#ifdef ANC_FB_ENABLED
            if ((ANC_FB_MIC_CH_L | ANC_FB_MIC_CH_R) & (AUD_CHANNEL_MAP_CH0 << i)) {
                int lr = (ANC_FB_MIC_CH_L & (AUD_CHANNEL_MAP_CH0 << i)) ? 0 : 1;
                if (adc_gain_offset[1][lr] < 0 && -adc_gain_offset[1][lr] > gain) {
                    gain = 0;
                } else {
                    gain += adc_gain_offset[1][lr];
                }
            }
#endif
#endif
...
void analog_aud_apply_adc_gain_offset(enum ANC_TYPE_T type, int16_t offset_l, int16_t offset_r)
{
    ...
    idx = (type == ANC_FEEDFORWARD) ? 0 : 1;

    if (adc_gain_offset[idx][0] == offset_l && adc_gain_offset[idx][1] == offset_r) {
        return;
    }

    TRACE("ana: apply adc gain offset: type=%d offset=%d/%d", type, offset_l, offset_r);

    adc_gain_offset[idx][0] = offset_l;
    adc_gain_offset[idx][1] = offset_r;
...
#endif
    val = SET_BITFIELD(val, REG_CODEC_ADC_IBSEL, 8);
