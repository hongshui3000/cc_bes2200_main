...
extern uint32_t thread_index;
#endif
static enum HAL_SLEEP_STATUS_T SRAM_TEXT_LOC NOINLINE hal_sleep_proc(int shallow_sleep)
{
    enum HAL_SLEEP_STATUS_T ret;
    uint32_t lock;
    uint32_t time = 0;
...
#ifdef HCI_DEBUG
            TRACE("CPU USAGE: busy=%d light_sleep=%d deep_sleep=%d, %d %d \n", 100 - (light_sleep_ratio + deep_sleep_ratio), light_sleep_ratio, deep_sleep_ratio, thread_index, thread_count);
#else		
