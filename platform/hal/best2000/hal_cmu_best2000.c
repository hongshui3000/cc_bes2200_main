...
void hal_cmu_timer_select_slow(void)
{
    ...
    // 32K
    cmu->PERIPH_CLK &= ~CMU_SEL_TIMER_FAST;
    cmu->PERIPH_CLK |= CMU_SEL_32K_WDT; //update ，WDT在32K下可以生效
    int_unlock(lock);
}
...
{ \
    ...
    if (freq == HAL_CMU_FREQ_32K) { \
        ...
        enable = CMU_SEL_OSC_ ##F## _ENABLE | CMU_SEL_OSCX2_ ##F## _ENABLE; \
        disable = CMU_SEL_PLL_ ##F## _DISABLE | CMU_RSTN_DIV_ ##F## _DISABLE | CMU_BYPASS_DIV_ ##F## _DISABLE; \
    } else { \
        enable = CMU_SEL_OSC_ ##F## _ENABLE | CMU_SEL_OSCX2_ ##F## _ENABLE | \
            CMU_SEL_PLL_ ##F## _ENABLE | CMU_BYPASS_DIV_ ##F## _ENABLE; \
...
{ \
    ...
    if (freq == HAL_CMU_FREQ_32K) { \
        ...
        enable = CMU_SEL_OSC_ ##F## _ENABLE | CMU_SEL_OSCX2_ ##F## _ENABLE | \
            CMU_SEL_PLL_ ##F## _ENABLE | CMU_RSTN_DIV_ ##F## _ENABLE; \
...
#if defined(USB_CLK_SRC_104M) || defined(USB_CLK_SRC_52M)
#ifdef USB_HIGH_SPEED
#error "USB HIGH-SPEED must use PLL"
#endif
#if defined(USB_CLK_SRC_104M) && !defined(ANA_26M_X4_ENABLE)
#error "USB_CLK_SRC_104M must use ANA_26M_X4_ENABLE"
#endif
    if (user == HAL_CMU_PLL_USER_USB) {
        return 0;
    }
#endif
...
void hal_cmu_codec_iir_disable(void)
{
    ...
    val = AON_CMU_EN_CODECIIR | AON_CMU_SEL_OSC_CODECIIR | AON_CMU_SEL_OSCX2_CODECIIR;
...
#else
    pll = HAL_CMU_PLL_USB;
#ifdef USB_CLK_SRC_104M
    src = CMU_USB_CLK_SRC_OSC_26M_X4;
#elif defined(USB_CLK_SRC_52M)
...
#if !defined(USB_HIGH_SPEED) && !defined(USB_CLK_SRC_52M)
...
void BOOT_TEXT_FLASH_LOC hal_cmu_module_init_state(void)
{
    ...
    aoncmu->CLK_SELECT |= AON_CMU_SEL_X4_BB; //解决X4模式下切换104M主频不稳
#endif
...
int SRAM_TEXT_LOC hal_cmu_lpu_sleep(void)
{
   ...
   // cmu->SYS_CLK_ENABLE = saved_clk_cfg;
    ...
    saved_clk_cfg |= (CMU_SEL_PLL_FLS_ENABLE | CMU_SEL_OSCX2_FLS_ENABLE);
    
    do
    {
        cmu->SYS_CLK_ENABLE = saved_clk_cfg;
    }while((cmu->SYS_CLK_ENABLE & saved_clk_cfg) != saved_clk_cfg);
