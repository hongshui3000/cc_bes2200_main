...
// Must enable audio PLL
// ch: 0-->left channel; 1-->right channel;
// gain: reduction gain value, step 2dB, max value 0x1f-->off
void hal_codec_sidetone_enable(int ch, int gain)
{
    codec->REG32B_05 = SET_BITFIELD(codec->REG32B_05, CODEC_32_ADC_DVST_CTL, gain);
    codec->REG32B_09 = SET_BITFIELD(codec->REG32B_09, CODEC_32_SIDETONE_MIC_SEL, ch);
}

void hal_codec_sidetone_disable(void)
{
    codec->REG32B_05 = SET_BITFIELD(codec->REG32B_05, CODEC_32_ADC_DVST_CTL, 0x1f);
}
...
#ifndef NO_CODEC_RESET
        codec->CMU_CTRL2 &= ~CODEC_CMU2_SOFT_RSTN_ADC_ALL;
        codec->CMU_CTRL2 |= CODEC_CMU2_SOFT_RSTN_ADC_ALL;
#endif
...
int hal_codec_setup_stream(enum HAL_CODEC_ID_T id, enum AUD_STREAM_T stream, const struct HAL_CODEC_CONFIG_T *cfg)
{
    ...
    if (stream == AUD_STREAM_PLAYBACK) {
        ...
        if (HAL_CODEC_CONFIG_SAMPLE_RATE & cfg->set_flag) {
            ...
            ana_dig_div = codec_dac_sample_rate[i].codec_div / codec_dac_sample_rate[i].cmu_div;
            ASSERT(ana_dig_div * codec_dac_sample_rate[i].cmu_div == codec_dac_sample_rate[i].codec_div,
                "%s: Invalid playback div for rate %u: codec_div=%u cmu_div=%u", __func__, cfg->sample_rate,
                codec_dac_sample_rate[i].codec_div, codec_dac_sample_rate[i].cmu_div);
            ...
            val = 0;
            if (hal_cmu_get_audio_resample_status() && codec_dac_sample_rate[i].codec_freq != CODEC_FREQ_26M) {
                val |= CODEC_10_RESAMPLE_ENABLE;
                if (codec_dac_sample_rate[i].codec_freq == CODEC_FREQ_44_1K_SERIES) {
            ...
            {
#ifdef __AUDIO_RESAMPLE__
                ASSERT(codec_dac_sample_rate[i].codec_freq != CODEC_FREQ_26M,
            ...
            hal_codec_set_dac_up(codec_dac_sample_rate[i].dac_up);
            hal_codec_set_dac_hbf_bypass_cnt(codec_dac_sample_rate[i].bypass_cnt);
        ...
        if (mic_map) {
            if (mic_map & AUD_CHANNEL_MAP_DIGMIC_ALL) {
                codec->REG32B_09 |= CODEC_09_PDM_ENABLE;
            } else if ((anc_adc_ch_map & AUD_CHANNEL_MAP_DIGMIC_ALL) == 0) {
                codec->REG32B_09 &= ~CODEC_09_PDM_ENABLE;
            }
...
#ifdef ANC_PROD_TEST
            if ((ANC_FF_MIC_CH_L > AUD_CHANNEL_MAP_CH4) || (ANC_FF_MIC_CH_L & (ANC_FF_MIC_CH_L - 1))) {
                ASSERT(false, "Invalid ANC_FF_MIC_CH_L: 0x%04X", ANC_FF_MIC_CH_L);
            }
            if ((ANC_FF_MIC_CH_R > AUD_CHANNEL_MAP_CH4) || (ANC_FF_MIC_CH_R & (ANC_FF_MIC_CH_R - 1))) {
...
#ifdef ANC_PROD_TEST
            if ((ANC_FB_MIC_CH_L > AUD_CHANNEL_MAP_CH4) || (ANC_FB_MIC_CH_L & (ANC_FB_MIC_CH_L - 1))) {
                ASSERT(false, "Invalid ANC_FB_MIC_CH_L: 0x%04X", ANC_FB_MIC_CH_L);
            }
            if ((ANC_FB_MIC_CH_R > AUD_CHANNEL_MAP_CH4) || (ANC_FB_MIC_CH_R & (ANC_FB_MIC_CH_R - 1))) {
...
#error "Invalid ANC_FB_MIC_CH_R"
        ...
        if (HAL_CODEC_CONFIG_SAMPLE_RATE & cfg->set_flag) {
            ...
            ana_dig_div = codec_adc_sample_rate[i].codec_div / codec_adc_sample_rate[i].cmu_div;
            ASSERT(ana_dig_div * codec_adc_sample_rate[i].cmu_div == codec_adc_sample_rate[i].codec_div,
            ...
            // 8K -> 4ms, 16K -> 2ms, ...
            adc_delay_ms = 4 / ((cfg->sample_rate + AUD_SAMPRATE_8000 / 2) / AUD_SAMPRATE_8000);
            if (adc_delay_ms < 2) {
                adc_delay_ms = 2;
            }
...
#ifdef __AUDIO_RESAMPLE__
            if (hal_cmu_get_audio_resample_status()) {
                ASSERT(codec_adc_sample_rate[i].codec_freq == CODEC_FREQ_26M,
...
int hal_codec_anc_adc_enable(enum ANC_TYPE_T type)
{
    ...
    if (anc_adc_ch_map & AUD_CHANNEL_MAP_DIGMIC_ALL) {
        codec->REG32B_09 |= CODEC_09_PDM_ENABLE;
    }
...
int hal_codec_anc_adc_disable(enum ANC_TYPE_T type)
{
    ...
    if ((anc_adc_ch_map & AUD_CHANNEL_MAP_DIGMIC_ALL) == 0 &&
            (codec_adc_ch_map & AUD_CHANNEL_MAP_DIGMIC_ALL) == 0) {
        codec->REG32B_09 &= ~CODEC_09_PDM_ENABLE;
    }
