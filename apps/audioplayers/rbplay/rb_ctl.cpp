...
osThreadDef(rb_ctl_thread, osPriorityHigh, 1024 * 3);
...
extern playlist_item *app_rbplay_get_playitem(uint16_t idx);
...
extern void rb_play_codec_init(void);
...
extern int app_ctl_remove_file(uint32_t idx);
...
int rb_ctl_is_audio_file(const char* file_path)
{
    unsigned int type = 0;
    type = probe_file_format(file_path);

    return (type == AFMT_UNKNOWN?0:1);
}
...
bool rb_ctl_parse_file(const char* file_path)
{
    ...
    app_rbplay_set_play_name(file_path);
...
void rb_ctl_stop_play(void )
{
    ...
    _LOG_DBG(" _rb_pcmbuf_w     _waiter_release \n");
    _rb_pcmbuf_r_waiter_release();
    _LOG_DBG(" _rb_pcmbuf_r     _waiter_release \n");
//close file
