...
#if defined(APP_TEST_AUDIO) && defined(ANC_TEST)
#include "anc_test.h"
...
{
    anc_tester((bool)param);
}

void test_usb_audio(APP_KEY_STATUS *status, void *param)
{
    usb_audio_tester((bool)param);
    //dualadc_audio_tester((bool)param);
}
#endif
#endif
...
void bt_change_to_jlink(APP_KEY_STATUS *status, void *param)
{
    //uint16_t value;
    TRACE("%s %d,%d",__func__, status->code, status->event);
    REG(0x40080004) = 0x00703818;
    REG(0x40087004) = 0xfff22222;
    REG(0x40085064) = 0x000025b2;
}
#endif
...
void bt_enable_tports(void)
{
    REG(0x40087004) = 0xaaaaaaaa;
    REG(0x40087014) = 0x000000ff;
    REG(0x4008701c) = 0x00000005;
}
#endif
...
int app_testcase_disp_menu(APP_KEY_HANDLE* testcase, bool printall)
{
    ...
    do{
        snprintf(buf, sizeof(buf), "|      (%d)%s", (int)testcase->key_status.code, testcase->string);
