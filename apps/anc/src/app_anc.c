...
extern void app_anc_status_sync_init(void);
...
int __anc_tester_fadein(enum ANC_TYPE_T type)
{
...
int __anc_tester_fadeout(enum ANC_TYPE_T type)
{
    int32_t gain0_curr, gain1_curr;
...
void anc_gain_fade_handle(void)
{
    ...
    if (app_anc_fade_status == APP_ANC_FADE_OUT)
    {
        ...
        while (__anc_tester_fadeout(ANC_FEEDFORWARD))
            ;
#endif
#ifdef ANC_FB_ENABLED
        while (__anc_tester_fadeout(ANC_FEEDBACK))
            ;
#endif
...
#ifdef ANC_FF_ENABLED
        while (__anc_tester_fadein(ANC_FEEDFORWARD))
            ;
#endif
#ifdef ANC_FB_ENABLED
        while (__anc_tester_fadein(ANC_FEEDBACK))
            ;
#endif
...
//anc_active_codec();
    ...
    anc_select_coef(anc_coef_idx, ANC_FEEDFORWARD, ANC_GAIN_DELAY);
#endif

#ifdef ANC_FB_ENABLED
    anc_open(ANC_FEEDBACK);
    anc_select_coef(anc_coef_idx, ANC_FEEDBACK, ANC_GAIN_DELAY);
#endif
...
static bool POSSIBLY_UNUSED sample_rate_44p1k_series(enum AUD_SAMPRATE_T rate)
{
    if (rate == AUD_SAMPRATE_22050 || rate == AUD_SAMPRATE_44100 ||
        rate == AUD_SAMPRATE_88200 || rate == AUD_SAMPRATE_176400 ||
        rate == AUD_SAMPRATE_352800)
    {
        return true;
    }
    return false;
...
static void anc_sample_rate_change(enum AUD_STREAM_T stream, enum AUD_SAMPRATE_T rate)
{
    ...
    if (anc_sample_rate[stream] != rate) {
        ...
        if (stream == AUD_STREAM_PLAYBACK) {
            play_rate = rate;
            cap_rate = anc_sample_rate[AUD_STREAM_CAPTURE];
        } else {
            play_rate = anc_sample_rate[AUD_STREAM_PLAYBACK];
            cap_rate = rate;
        }
        // No need to update coef type
#else
        bool rate_44p1k;

        rate_44p1k = sample_rate_44p1k_series(rate);
        if (stream == AUD_STREAM_PLAYBACK) {
            play_rate = rate;
            if (sample_rate_44p1k_series(anc_sample_rate[AUD_STREAM_CAPTURE]) == rate_44p1k) {
                cap_rate = anc_sample_rate[AUD_STREAM_CAPTURE];
            } else {
                cap_rate = rate_44p1k ? AUD_SAMPRATE_44100 : AUD_SAMPRATE_48000;
            }
        } else {
            cap_rate = rate;
            if (sample_rate_44p1k_series(anc_sample_rate[AUD_STREAM_PLAYBACK]) == rate_44p1k) {
                play_rate = anc_sample_rate[AUD_STREAM_PLAYBACK];
            } else {
                play_rate = rate_44p1k ? AUD_SAMPRATE_44100 : AUD_SAMPRATE_48000;
            }
        }
        anc_set_coef_type(sample_rate_44p1k_series(rate));
#endif
#endif
        ...
        af_anc_setup(play_rate, cap_rate);
...
void app_anc_open_anc(void)
{
    ...
    if (anc_sample_rate[AUD_STREAM_PLAYBACK] == AUD_SAMPRATE_NULL) {
        ...
        anc_sample_rate[AUD_STREAM_PLAYBACK] = AUD_SAMPRATE_50781;
#endif
#ifdef SW_CAPTURE_RESAMPLE
        anc_sample_rate[AUD_STREAM_CAPTURE] = AUD_SAMPRATE_50781;
#endif
    ...
    anc_set_coef_type(sample_rate_44p1k_series(playback_rate));
#endif
...
void app_anc_status_change(void)
{
    ...
    case ANC_STATUS_OFF:
        ...
        break;
    case ANC_STATUS_ON:
#ifdef ANC_COEF_SWITCH_NEXT
        if (anc_coef_idx < (ANC_COEF_LIST_NUM - 1))
        {
...
static int app_anc_handle_process(APP_MESSAGE_BODY *msg_body)
{
    ...
    case ANC_EVENT_HOWL_PROCESS:
        //disable anc,set max gain,enable anc
        {
            ...
            {
                //__anc_tester_howl_set_gain(howl);
            }
        }
        break;
#ifdef __COWIN_V2_BOARD_ANC_SINGLE_MODE__
...
void app_anc_key(APP_KEY_STATUS *status, void *param)
{
    ...
    app_anc_status_change();
#ifdef __TWS__
    app_anc_status_sync(anc_work_status);
#endif
}
